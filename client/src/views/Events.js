...
class Events extends React.Component<Props> {
  ...
  async loadData() {
    ...
    const result = await fetch(
      `/api/events?limit=${limit}&skip=${page * limit}`,
      {
        method: 'GET'
      },
    );
    ...
    const columns = [
      { name: 'FIO', title: 'Фамилия' },
      { name: 'NAME', title: 'Место' }
    ];
    const rows = data.map(item => {
      const object = {
        id: item.ID,
        date: moment(item.DT).format('DD/MM HH:mm'),
        events: this.getEvent(item.EVN),
        actions: (
    ...
    columns.push({ name: 'events', title: 'Событие' });
      ...
      <>
        <Container fluid className='main-content-container px-4'>
          <Row noGutters className='page-header py-4'>
            <PageTitle sm='4' subtitle='События' className='text-sm-left' />
          </Row>
          <Row>
            <Col>
        ...
        {modalIsOpen && (<Event onClose={this.closeHandler} event={current} getEvent={this.getEvent} />)}
      </>
    );
  }
}
